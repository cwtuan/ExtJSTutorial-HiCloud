...
Ext.define('MyApp.ux.grid.column.ComponentColumn', {
	...
	/*
	 * Defer durations for updating the component width when a column resizes. Required when a component has an animated resize that causes the scrollbar to
	 * appear/disappear. Otherwise the animated component can end up the wrong size.
	 * 
	...
	addRefOwner : function(child) {
		var me = this, fn = me.refOwnerFn || (me.refOwnerFn = function() {
			return me;
		});
	...
	/*
	 * In IE setting the innerHTML will destroy the nodes for the previous content. If we try to reuse components it will fail as their DOM nodes will have been
	 * torn apart. To defend against this we must remove the components from the DOM just before the grid view is refreshed.
	 */
	...
	calculateFrameWidth : function(component) {
		var el = component.getEl(), parentDiv = el && el.parent(),
	...
	getRefItems : function(deep) {
		var items = this.callParent([ deep ]), ids = this.compIds, index = 0, len = ids.length, item;
	...
	onColumnVisibilityChange : function(column) {
		var items = column.getRefItems(), index = 0, length = items.length, visible = !column.isHidden();
	...
	performGC : function() {
		var compIds = this.compIds, index = compIds.length - 1, comp, el;
	...
	processValue : function(value) {
		var me = this, compIds = me.compIds, id, initialWidth, dom, parent;
		...
		if (value && value.isComponent) {
			...
			if (value.rendered) {
				/*
				 * This is only necessary in IE because it is just another manifestation of the innerHTML problems. The problem occurs when a record value is
				 * changed and the components in that same row are being reused. The view doesn't go through a full refresh, instead it performs a quick update
				 * on just the one row. Unfortunately this nukes the existing components so we need to remove them first.
				 */
				if (Ext.isIE) {
				...
				/*
				 * Set the width to a 'best guess' before the component is rendered to ensure that the component's layout is using a configured width and not
				 * natural width. This avoids problems with 4.1.1 where subsequent calls to setWidth are ignored because it believes the width is already
				 * correct but only the outermost element is actually sized correctly. We could use an arbitrary width but instead we make a reasonable guess at
				 * what the actual width will be to try to avoid extra resizing.
				 */
	...
	resizeChild : function(component, defer) {
		var me = this, frameWidth, newWidth, oldWidth, resizeQueue;
	...
	resumeResizing : function() {
		var me = this, index = 0, resizeQueue = me.resizeQueue, len = resizeQueue.length;
	...
	var proto = cls.prototype, version = Ext.getVersion();
