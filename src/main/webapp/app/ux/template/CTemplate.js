...
Ext.define('MyApp.ux.template.CTemplate', {
	...
	/*
	 * Takes a recursive copy of the values provided, switching out components for placeholder values. The component ids are recorded and injectComponents()
	 * uses the ids to find the placeholder elements in the DOM and switch in the components.
	 */
	copyValues : function(values, depth) {
		var me = this, id, copy = {}, copyDepth = depth || me.copyDepth;
	...
	/*
	 * We have to resort to polling for component injection as we don't have full control over when the generated HTML will be added to the DOM
	 */
	...
	/*
	 * Attempts to substitute all placeholder elements with the real components. If a component is successfully injected or it has been destroyed then it won't
	 * be attempted again. This method is repeatedly invoked by a polling mechanism until no components remain, however relying on the polling is not advised.
	 * Instead it is preferable to call this method directly as soon as the generated HTML is inserted into the DOM.
	 */
	injectComponents : function() {
		var me = this, ids = me.ids, index = ids.length - 1, id, cmp, placeholderEl;
	...
	overwrite : function(el) {
		var dom, firstChild, ret;

		/*
		 * In IE setting the innerHTML will destroy the nodes for the previous content. If we try to reuse components it will fail as their DOM nodes will have
		 * been torn apart. We can't defend against external updates to the DOM but we can guard against the case where all updates come through this template.
		 */
		if (Ext.isIE) {
			dom = Ext.getDom(el);
