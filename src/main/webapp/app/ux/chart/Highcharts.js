 ...
 * 
 * A much improved & ported from ExtJs 3 Highchart adapter. - Supports the latest Highcharts (3.0.0) - Supports both Sencha ExtJs 4 and Touch 2 - Supports
 * Highcharts animations
 * 
 * In order to use this extension, you are expected to know how to use Highcharts and Sencha products (ExtJs 4 &amp; Touch 2). # Configuring Highcharts
 * Extension The Highcharts extension requires a few changes from an existing Highcharts configuration. Suppose we already have a configuration as follows:
 * @example var chart = new Highcharts.Chart({ chart: { renderTo: 'container', type: 'spline' }, title: { text: 'A simple graph' }, xAxis: { categories: [
 *          'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec' ] }, series: [{ dashStyle: 'DashDot', data: [54.7, 54.7, 53.9,
 *          54.8, 54.4, 54.2, 52.4, 51.0, 49.0, 47.4, 47.0, 46 ] }] }); ## Step 1: Remove data related fields
 * 
 * The first step is to take out the configuration object and remove any data related properties such as: *xAxis.categories* and *series[0].data*. Then removes
 * *chart.renderTo* option as the extension will fill in that property internally. This leaves us with the following config:
 * @example chart: { type: 'spline' }, title: { text: 'A simple graph' }, series: [{ dashStyle: 'DashDot' }] ## Step 2: Create chartConfig
 * 
 * The next step is to create an object called, *chartConfig*, and put the above configuration in it. Then we extract the series array to an upper level which
 * gives the followings:
 * @example series: [{ dashStyle: 'DashDot' }], chartConfig: { chart: { type: 'spline' }, title: { text: 'A simple graph' } } ## Step 3: Create ExtJs Store and
 *          data mappings
 * 
 * Then we create a ExtJs Store object to map the data fields.
 * @example Ext.define('SampleData', { extend: 'Ext.data.Model', fields: [ {name: 'month', type: 'string'}, {name: 'value', type: 'float'} ] });
 * 
 * var store = Ext.create('Ext.data.Store', { model: 'SampleData', proxy: { type: 'ajax', url: '/getData.php', reader: { type: 'json', root: 'rows' } },
 * autoLoad: false }); Then we modify the series array with data mappings to Store; we add *xField* outside the series array as categories data and *dataIndex*
 * for the y-axis values. For historical reason, we can also use *yField*, just an alias name for *dataIndex*.
 * @example series:[{ dashStyle: 'DashDot', dataIndex: 'value' }], xField: 'month', store: store, chartConfig: { chart: { .... ## Step 4: Create ExtJs
 *          Highcharts Component
 * 
 * The final step is to create a Highcharts component with the whole config as an object specifier.
 * @example var win = new Ext.create('Ext.window.Window', { layout: 'fit', items: [{ xtype: 'highchart', series:[{ dashStyle: 'DashDot', dataIndex: 'value' }],
 *          xField: 'month', store: store, chartConfig: { chart: { type: 'spline' }, title: { text: 'A simple graph' } } }] }).show(); # Updating Highcharts
 *          chart properties dynamically Some of the Highcharts properties cannot be updated interactively such as relocating legend box, switching column
 *          charts stacking mode. The only way is to manually destroy and create the whole chart again. In Highcharts extension, this can be done in an easier
...
Ext.define("MyApp.ux.chart.Highcharts", {
	...
	statics : {
		/*******************************************************************************************************************************************************
		 * @static Version string of the current Highcharts extension
		 */
		version : '2.4.5',

		/*******************************************************************************************************************************************************
		 * @property {Object} sencha
		 * @readonly Contain shorthand representations of which Sencha product is the Highcharts extension currently running in. // Under Sencha ExtJs {
		 *           product: 'e', major: 4, name: 'e4' } // Under Sencha Touch 2 { product: 't', major: 2, name: 't2' }
		 */
		sencha : function() {
	...
	/***********************************************************************************************************************************************************
	 * @property {Boolean} debug Switch on the debug logging to the console
	 */
	debug : false,
	...
	/***********************************************************************************************************************************************************
	 * This method is called by other routines within this extension to output debugging log. This method can be overrided with Ext.emptyFn for product
	 * deployment
	 * 
	 * @param {String}
	 *            msg debug message to the console
	 */
	log : function(msg) {
	...
	/**
	 * @cfg {Object} defaultSerieType If the series.type is not defined, then it will follow the precedence of Highcharts (inside chartConfig) chart.type ->
	 *      Highcharts chart.defaultSeriesType -> and finally this option
	 */
	defaultSerieType : 'line',

	/**
	 * @cfg {Boolean} resizable True to allow resizing, false to disable resizing (defaults to true).
	 */
	resizable : true,
	...
	/**
	 * @cfg {Object} loadMask An {@link Ext.LoadMask} config or true to mask the chart while loading. Defaults to false.
	 */
	loadMask : false,
	...
	/**
	 * @cfg {Boolean} refreshOnChange chart refresh data when store datachanged event is triggered, i.e. records are added, removed, or updated. If your
	 *      application is just purely showing data from store load, then you don't need this.
	 */
	...
	/**
	 * @cfg {Boolean} this config enable or disable chart animation
	 */
	animation : false,
	updateAnim : false,

	/***********************************************************************************************************************************************************
	 * @cfg {Boolean} lineShift The line shift is achieved by comparing the existing x values in the chart and x values from the store record and work out the
	 *      extra record. Then append the new records with shift property. Hence, any old records with updated y values are ignored
	 */
	lineShift : false,

	initAnim : false,
	/**
	 * @cfg {Boolean} In a nutshell, keeps this option to true.
	 * 
	 * Since Highcharts initial and update animations are not the same, if you want to make sure there is initial animation, then you should create store and
	 * extension in specific sequence. First, set the {@link Ext.data.Store#cfg-autoLoad} option to false, create the Highcharts component with the store, then
	 * call the {@link Ext.data.Store#method-load} method. The *initAnimAfterLoad* defers creating the chart internally until the store is loaded. Disabling it,
	 * the extension will create the chart instantly and you will only see the update animation after the load.
	 */
	initAnimAfterLoad : false,

	/**
	 * @cfg {Function} afterChartRendered callback for after the Highcharts is rendered. **Note**: Do not call initial {@link Ext.data.Store#method-load} inside
	 *      this handler, especially with *initAnimAfterLoad* set to true because {@link Ext.data.Store#method-load} will never be called as the chart is
	 *      deferring to render waiting for the store data. Here is an example of how this should be called. This 'this' keyword refers to the Highcharts ExtJs
	 *      component whereas chart refers to the created Highcharts chart object items: [{ xtype: 'highchart', listeners: { afterChartRendered: function(chart) { //
	 *      'this' refers to the 'highchart' ExtJs component var size = this.getSize(); // Get the average value of the first series var temp = 0;
	 *      Ext.each(chart.series[0].data, function(data) { temp += data; }); temp = temp / chart.series[0].data.length; Ext.Msg.alert('Info', 'The average
	 *      value is ' + temp); } }, series:[ ... ], xField: 'month', store: store, chartConfig: { .... }
	 */
	...
	/***********************************************************************************************************************************************************
	 * Add one or more series to the chart. The addSeries method can be used with Serie field name configurations referring to fields from the store or static
	 * data using the data field as the native Highcharts series configuration // Append a series with specific data addSeries([{ name: 'Series A', data: [ [ 3,
	 * 5 ], [ 4, 6 ], [ 5, 7 ] ] }], true);
	 * 
	 * @param {Array}
	 *            series An array of series configuration objects
	 * @param {Boolean}
	 *            append Append the series if true, otherwise replace all the existing chart series. Optional parameter, Defaults to true if not specified
	 */
	 ...
	 * 
	 * @param {Number}
	 *            id the index value in the chart series array
	 * @param {Boolean}
	 *            redraw Set it to true to immediate redraw the chart to reflect the change
	 */
	...
	/***********************************************************************************************************************************************************
	 * Remove all series in the chart. This also remove any categories data along the axes
	 */
	...
	/***********************************************************************************************************************************************************
	 * @private Build the initial data set if there are data already inside the store.
	 */
	buildInitData : function() {
		...
		if (!this.store || this.store.isLoading() || !_this.chartConfig || _this.initAnim === false || _this.chartConfig.chart.animation === false) {
			return;
		}
	...
	/**
	 * Redraw the chart. It internally destroys existing chart (if already display) and re-creates the chart object. Call this method to reflect any structural
	 * changes in chart configuration
	 */
	draw : function() {
		// Sencha Touch uses config to access properties
		var _this = (this.statics().sencha.product == 't') ? this.config : this;
	...
	/**
	 * Complete refresh series in the chart. This method rebuilds the chart series array from the current store records. Any store record changes should call
